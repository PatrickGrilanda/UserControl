<template>
  <div class="lg:grid lg:grid-cols-5 lg:gap-4">
    ...
    <main class="col-span-3 w-full">
      <div class="bg-white rounded-lg shadow border p-2">
    ...
    <aside class="hidden lg:block rounded-lg h-full max-h-fit">
      <div class="flex flex-col gap-2">
        <div class="flex flex-col gap-2">
          ...
          <ul class="flex flex-col gap-2">
            <li v-for="friendshipRequest in friendshipRequests" :key="friendshipRequest.id">
              <div class="grid grid-cols-3 items-center">
                <div class="col-span-2 flex">
                  <img :src="friendshipRequest.from.image" class="w-10 h-8 rounded-full">
                  <span class="ml-2">{{ friendshipRequest.from.name }}</span>
                </div>
...
export default {
  ...
  methods: {
    getFriendsPublications() {
      ...
      api.get('/user_publications' + this.mountQuery(Array.from(this.user.friends), 'user_id'))
        .then((response) => {
